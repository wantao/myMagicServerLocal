...
*/
bool mvCSocketBase::mvCreateSocket(unsigned int& nSocket, int nType) //1:tcp; 2:udp
{
#ifdef SOCKET_BASE_LOG
...
*/
bool mvCSocketBase::mvPrepareSocket(unsigned int& nSocket)
{
    //先关闭套接字
...
* 返回值 ：成功返回true，否则false
*/
bool mvCSocketBase::mvBindPort(unsigned int& nSocket, int nPort)
{
#ifdef SOCKET_BASE_LOG
...
* 返回值 ：成功返回true，否则false
*/
bool mvCSocketBase::mvSetSocketOpt(unsigned int& nSocket, int nOpt)
{
#ifdef SOCKET_BASE_LOG
...
* 返回值 ：成功返回true，否则false
*/
bool mvCSocketBase::mvStartListen(unsigned int& nSocket)
{
#ifdef SOCKET_BASE_LOG
...
* 返回值 ：成功返回true，否则false
*/
bool mvCSocketBase::mvWaitConnect(unsigned int& nSocket, const string& strHost, int nPort, int nSec)
{
#ifdef SOCKET_BASE_LOG
...
* 返回值 ：成功返回true，否则false
*/
bool mvCSocketBase::mvSendMsgToSocket(unsigned int& nSocket, const string& strMsg,bool bBlock)
{
#ifdef SOCKET_BASE_LOG
    ...
    while (nLeft > 0)
    {
        if(!bBlock)
        {
            if (!mvWaitToWriteSocket(nSocket))
            {
					...
					printf("mvWaitToWriteSocket==nLeft=%d,nDataLen=%d,%s\n",nLeft,nDataLen,strerror(errno));
                    return false;
            }
        }
        ...
        if ((nBytes = send(nSocket, strMsg.c_str()+nDataLen, nLeft>16384?16384:nLeft, MSG_NOSIGNAL)) < 0)
        {
           ...
           // LogError("send error===nSocket=%d==nLeft=%d,nDataLen=%d,%s\n",nSocket, nLeft,nDataLen,strerror(errno));
			printf("send error===nSocket=%d==nLeft=%d,nDataLen=%d,%s\n",nSocket, nLeft,nDataLen,strerror(errno));
            return false;
        }
        nDataLen += nBytes;
        nLeft -= nBytes;
...
* 返回值 ：无
*/
void mvCSocketBase::mvCloseSocket(unsigned int &nSocket)
{
#ifdef SOCKET_BASE_LOG
...
* 返回值 ：成功返回true，否则返回false
*/
bool mvCSocketBase::mvWaitToWriteSocket(unsigned int& nSocket)
{
#ifdef SOCKET_BASE_LOG
...
* 返回值 ：无
*/
void mvCSocketBase::mvSetSocketBlock(unsigned int &nSocket, bool bBlock)
{
#ifdef SOCKET_BASE_LOG
