...
//bool getMaxMissionIdByUserId(const unsigned int& u_userId,int& u_maxMissionId)
//{
//	char buf[128] = {0};
//	sprintf(buf,"select max(missionId) from m_userMissionsInfo%d where userId = %d;",
//		u_userId%USER_MISSION_TABLE_COUNT,u_userId);
//	string strSql(buf);
//	MysqlQuery q = g_cMagicServerDbRead.master_slave_exQuery(strSql);
//	if (q.eof())
//	{
//		g_cMyGlog.errorlogRecord("getMaxMissionIdByUserId error,strSql:%s\n",strSql.c_str());
//		q.finalize();
//		return false;
//	}
//	u_maxMissionId = q.getIntFileds("max(missionId)");
//	q.finalize();
//	return true;
//} 
