...
//背包扩充设置
int cProcUserShopBuy::setAddBag(const unsigned int& u_userId)
{
	int u_result = 0;
	...
	u_bagCount += ONE_TIME_ADD_BAG_COUNT;
...
int cProcUserShopBuy::setBuyEnergy(const unsigned int& u_userId)
{
	...
	{
		//vip特权判断
		u_result = isUserToMaxBuyEnergyCount(u_userId);
		if (u_result != 0)
		{
			return u_result;
		}
...
string cProcUserShopBuy::getPayMissionCondition(const int& u_userMoneyTotal,const int& u_moneyAdd)
{
	...
	else if (u_totalMoney >= M_1500_YUAN_CONDITION && u_totalMoney < M_3000_YUAN_CONDITION)
	{
		u_condition = M_1500_YUAN_CONDITION;
	}
	else if (u_totalMoney >= M_3000_YUAN_CONDITION && u_totalMoney < 3500)
	{
...
//购买魔石处理
int cProcUserShopBuy::setBuyMagicSton(const unsigned int& u_userId,const int& u_buyType)
{
	int u_result = 0;
...
int cProcUserShopBuy::proc91ShopBuy(const unsigned int& u_userId,const int& u_buyType)
{
	...
	switch(u_buyType)
	{
		g_cMyGlog.errorlogRecord("proc91ShopBuy:Type :%d\n",u_buyType);
		case ADD_BAG:
			u_result = setAddBag(u_userId);
			break;
		case BUY_6_YUAN_MAGIC_STON:
			u_result = setBuyMagicSton(u_userId,BUY_6_YUAN_MAGIC_STON);
			break;
		case BUY_36_YUAN_MAGIC_STON:
			u_result = setBuyMagicSton(u_userId,BUY_36_YUAN_MAGIC_STON);
			break;
		case BUY_98_YUAN_MAGIC_STON:
			u_result = setBuyMagicSton(u_userId,BUY_98_YUAN_MAGIC_STON);
			break;
		case BUY_388_YUAN_MAGIC_STON:
			u_result = setBuyMagicSton(u_userId,BUY_388_YUAN_MAGIC_STON);
			break;
		case BUY_648_YUAN_MAGIC_STON:
			u_result = setBuyMagicSton(u_userId,BUY_648_YUAN_MAGIC_STON);
			break;
		default:
			u_result = -1;
			break;
...
int cProcUserShopBuy::getGoodsIdByGoodsOrder(const PBIAPSend& u_PBIAPSend)
{
	int u_goodsId = 0;
	char buf[128] = {0};
	sprintf(buf,"select goodsId from m_order_table where cooOrderSerial='%s';",(u_PBIAPSend.orderid()).c_str());
	string strSql(buf);
	MysqlQuery q = g_cMagicServerDbRead.master_slave_exQuery(strSql);
	if (q.eof())
	{
		g_cMyGlog.errorlogRecord("getGoodsIdByGoodsOrder error,strSql:%s\n",strSql.c_str());
		q.finalize();
		return u_goodsId;
	}
...
int cProcUserShopBuy::proc91PBIAPRecv(PBIAPRecv& u_PBIAPRecv,const unsigned int& u_userId,const PBIAPSend& u_PBIAPSend)
{
	...
	int u_goodsId = getGoodsIdByGoodsOrder(u_PBIAPSend);
	if (u_goodsId == 0)
	{
		u_PBIAPRecv.set_result(1);	
	}
	else
	{
...
int cProcUserShopBuy::procIOSPBIAPRecv(PBIAPRecv& u_PBIAPRecv,const unsigned int& u_userId,const PBIAPSend& u_PBIAPSend)
{
	...
	if(curl) {
		...
		if(CURLE_OK == res) {
			...
			u_result = m_cMyJsonParser.procJsonData(u_PBIAPSend.macaddress(),u_stream/*,u_PBIAPSend.serverurl()*/);
			if (u_result == -1)
			{
										  ...
										  int& u_energy,const int& u_userLevel)
{
	...
	int u_maxEnergy = USER_MAX_ENERGY(u_userLevel);//ENERGY + (u_userLevel-1)/2;
