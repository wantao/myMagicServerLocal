...
void cUpdateUserBaseInfo::insertUserActiveBattleInfo(const int& u_userId,const activeBattleMap& u_activeBattleMap)
{
	...
	if (u_insertList.size() > 7)
	{
		...
		{
			g_cMyGlog.errorlogRecord("insertUserActiveBattleInfo error:strSql:%s\n",strSql.c_str());
		}
	}
}
...
//清空用户当日扫荡次数和购买体力药水的次数
void cUpdateUserBaseInfo::clearUserWipeOutAndBuyEnergyCount()
{
	for (int i = 0; i < 20; i++)
	{
		char buf[128] = {0};
		sprintf(buf,"delete from m_user_wipeout_enery_count%d;",i);
		string strSql(buf);
		if (g_cMagicServerDb.execSQL(strSql) != 0)
		{
			g_cMyGlog.errorlogRecord("clearUserWipeOutAndBuyEnergyCount error,strSql:%s\n",strSql.c_str());
		}
	}
}
