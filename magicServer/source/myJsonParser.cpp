...
bool cMyJsonParser::isIosOrderProcced(const string& u_transaction_id)
{
	char buf[128] = {0};
	bool retFlag = false;
...
int cMyJsonParser::getBuyTypeByProcductId(const string& u_procductId)
{
	...
	if (q.eof())
	{
		g_cMyGlog.debugLog("updateUserInfoByProcductId error,strSql:%s\n",strSql.c_str());
	}
	else
	{
...
int cMyJsonParser::procJsonData(const string& u_maccAddress,const string& u_jsonData/*,const string& u_serverUrl*/)
{
	using namespace std;
	...
	if (reader.parse(u_jsonData, value))
	{
		...
		if (n_status != 0)
		{
			g_cMyGlog.debugLog("n_status:%d\n",n_status);
			return u_result;
		}
		const Json::Value arrayObj = value["receipt"];
		if (isIosOrderProcced(arrayObj["transaction_id"].asString()))
		{
			//g_cMyGlog.debugLog("transaction_id:%s has been procced\n",arrayObj["transaction_id"].asString().c_str());
			u_result = 0;
			return u_result;
		} 
		...
		unsigned int n_userId = getUserIdByUserMacAddress(u_maccAddress);
		u_result = g_cProcUserShopBuy.proc91ShopBuy(n_userId,n_buyType);
		if (u_result == -1)
		{
			return u_result;
		}
