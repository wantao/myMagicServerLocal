...
public:
	...
	//背包扩充设置
	int setAddBag(const unsigned int& u_userId,const int& u_addNum,const int& u_needMoney);
	//获取体力药水计数
	...
	//购买魔石处理
	int setBuyMagicSton(const unsigned int& u_userId,const int& u_buyType,const int& u_addDiamonds,const int& u_needMoney);
	//获取用户复活次数
	...
	int getAddNumByBuyType(const int& u_buyType,int& u_needMoney);
	//处理91商店购买请求
	int proc91ShopBuy(const unsigned int& u_userId,const int& u_buyType);
	int getGoodsIdByGoodsOrder(const PBIAPSend& u_PBIAPSend,int& u_isOrderProceed);
	...
	bool isIosOrderProcced(const string& u_transaction_id);
	//获取体力恢复倒计时
	...
	//获取支付服务器的url
	string getPayServerUrlByGameServerUrl(const string& u_gameServerUrl);
